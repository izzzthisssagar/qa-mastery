...
{/* IMAGE: a restaurant pass/kitchen window with order tickets in a row and plates being handed out */}
<HotspotImage
  src="/notes/browser-devtools-mastery/network/PLACEHOLDER-kitchen-pass.jpg"
  alt="A busy restaurant kitchen pass with a rail of order tickets and plates queued under heat lamps waiting for service"
  credit="Photo: PLACEHOLDER - replace with a restaurant kitchen pass: a rail of paper order tickets above, finished plates queued under heat lamps, a chef reaching for the next ticket"
  pins={[
    { x: 50, y: 50, label: "The ticket rail = the waterfall", desc: "Orders arrive, queue, and get worked in overlapping batches - exactly the panel's waterfall stripe. Each Network row's bar shows when its request started and how long each phase lasted; stacked bars reveal what ran in parallel and what waited. Reading the waterfall answers 'slow WHERE': one giant bar (a single slow request) is a different bug from a staircase of short bars (requests queuing behind each other, often waiting on the one before)." },
    { x: 50, y: 50, label: "The kitchen working = TTFB", desc: "Between ticket-in and first-plate-out, the customer sees nothing - that gap is Waiting for server response in the Timing tab. It is backend time: queries, application logic, upstream calls. A fat TTFB with a small download means the server thought too long; no image compression or front-end tune-up will shorten the kitchen. This is the number that assigns slowness to the backend team with evidence instead of vibes." },
    { x: 50, y: 50, label: "Plates crossing the room = content download", desc: "The kitchen may be instant while the carrying takes forever: a 5 MB response dribbling over hotel wifi is all download time. In the Timing tab this is the content download phase; the Size column names the accomplice. Big size plus long download plus healthy TTFB means the payload is the problem - oversized images, unpaginated API responses returning 10,000 rows for a screen that shows ten." },
    { x: 50, y: 50, label: "The regular's usual = cache hits", desc: "Some orders never reach the kitchen - the barista starts the usual on sight. Rows whose Size column says (from memory cache) or (from disk cache) never touched the network; 304 rows asked and were told 'use your copy'. Free speed - until you are RETESTING A FIX against a cached copy of the bug. Tick Disable cache when verifying fixes: test the server's truth, not your browser's souvenir." },
    { x: 50, y: 50, label: "The wrong dish delivered = Content-Type mismatch", desc: "The ticket says soup; the plate is salad. When the Content-Type response header claims one thing and the body IS another - an HTML login page served to code expecting application/json is the classic - clients crash or render garbage. Verify the claim against the Response tab's actual body: 'Unexpected token < in JSON' in the console is this exact dish, and the panel shows both the lying header and the true body side by side." },
  ]}
/>
...
<Resources
  links={[
    ...
    { href: "PLACEHOLDER", title: "PLACEHOLDER video - Timing tab and waterfall walkthrough: TTFB vs download on a real slow page", kind: "watch" },
  ]}
/>

<Video href="PLACEHOLDER" title="PLACEHOLDER - status families, the Timing tab, and reading the waterfall" minutes={11} />
