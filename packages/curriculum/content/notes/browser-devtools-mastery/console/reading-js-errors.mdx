...
<HotspotImage
  src="/notes/browser-devtools-mastery/console/PLACEHOLDER-console-panel-error.jpg"
  alt="Chrome DevTools Console panel open with a red TypeError entry expanded, showing the error message, a multi-line stack trace, and a clickable file colon line link on the right edge of the entry"
  credit="Photo: PLACEHOLDER - replace with a close-up screenshot of Chrome DevTools Console panel showing an expanded red TypeError with its stack trace and clickable file:line link"
  pins={[
    { x: 50, y: 50, label: "The name - what kind of thing broke", desc: "The bold red word at the start of the entry, TypeError, ReferenceError, or SyntaxError. This alone tells a trained eye the shape of the bug before reading another word: a TypeError smells like missing or reshaped data, a ReferenceError smells like a typo or a 404'd script, a SyntaxError smells like a broken deploy. Read this word first, every time." },
    { x: 50, y: 50, label: "The message - the specific detail", desc: "The text after the name, such as Cannot read properties of undefined (reading 'total'). This names the exact value that misbehaved and what the code asked of it. Quote it verbatim in any report - it is the single most useful sentence in the whole entry, and paraphrasing it throws away information the developer would otherwise get for free." },
    { x: 50, y: 50, label: "The clickable file:line link", desc: "Top-right of the entry, formatted like cart.js:44. This is not decoration - it is a live link into the Sources panel. Click it once and DevTools opens the exact line, cursor in place, the failing expression highlighted. A tester who reports 'the file' instead of clicking this link is doing the developer's search for them, badly." },
    { x: 50, y: 50, label: "The stack frames below - the call chain", desc: "Each indented at line under the top one names a function that was mid-call when the crash happened, most recent first. You do not need to trace every frame - but noting the FIRST one that belongs to the app's own code (not a library or node_modules) tells you where a human decision, not a library internal, produced the bad value." },
    { x: 50, y: 50, label: "The little grey triangle - collapsed context", desc: "Many entries arrive collapsed, hiding extra frames or a logged object behind a small disclosure triangle. Testers in a hurry report only what is visible and miss the expanded detail underneath - always click to expand before deciding an error is 'simple'. The full picture is one click away, same as the source link." },
  ]}
/>
...
<Resources
  links={[
    ...
    { href: "PLACEHOLDER", title: "PLACEHOLDER video - reading the Console panel's error anatomy live", kind: "video" },
  ]}
/>

<Video url="PLACEHOLDER" title="PLACEHOLDER video - reading the Console panel's error anatomy live" minutes={8} />
