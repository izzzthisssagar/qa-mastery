...
<Term term="accessible name">
The label a browser computes for an element and hands to assistive technology — what a
screen reader actually says out loud. It comes from the element's text content, or its
`<label>`, or an `aria-label`, in a defined order of precedence. An icon-only button with
no text and no `aria-label` has *no* accessible name: it announces as bare "button" and
the user must guess. It is also the `name` in `getByRole('button', { name: 'Add to cart' })`,
which is why that locator and a screen reader always agree with each other. Read the
computed value in DevTools → Elements → Accessibility pane.
</Term>
