...
<Callout type="tip">
Fastest way to answer "whose move is it?" without memorizing the graph: read each state as *who acts
next*, not who acted last. "Fixed" doesn't mean "the developer is done" — it means "a tester now owes
a retest." "Pending Retest" means the ball is in QA's court; "Assigned" means it's the developer's.
Say the state out loud as "waiting on \_\_\_" and the owner falls right out — no diagram required.
</Callout>
