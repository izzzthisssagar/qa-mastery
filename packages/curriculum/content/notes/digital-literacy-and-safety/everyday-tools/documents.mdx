...
<Term define={`Data about the data — everything a file records that isn't the content you typed. For documents this includes the author's name, their organisation, the machine's user account, creation and modification timestamps, total editing time, the template used, every comment (resolved or not), tracked changes, and the full-resolution originals of any image you "cropped". It is not hidden maliciously; it exists to make the software work. The failure is that every interface shows you the *page* while the *file* is what gets sent — so people form an accurate mental model of the wrong object.`}>metadata</Term>
