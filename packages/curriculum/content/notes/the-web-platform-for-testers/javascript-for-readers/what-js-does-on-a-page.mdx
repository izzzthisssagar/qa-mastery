...
<HotspotImage
  ...
  pins={[
    { x: 30, y: 18, label: "function — a named block of behaviour", desc: "A recipe with a name. It does nothing until something CALLS it. Half of 'the button does nothing' bugs are a perfectly correct function that nobody ever called, and you can see that in the console: no error, no request, nothing happened at all." },
    { x: 55, y: 40, label: "Variables hold state", desc: "`const` never changes, `let` does, `var` is the old one you'll meet in old code. When a test sees stale data on screen, some variable is holding yesterday's value — and the fix is never in the HTML." },
    { x: 40, y: 62, label: "Braces { } mark a block", desc: "Everything between them belongs together and runs together. If a script throws an error partway through a block, the rest of that block never runs — which is why one early error can leave a page half-wired and silent." },
    { x: 65, y: 78, label: "The semicolons and the noise", desc: "Punctuation. Genuinely ignorable for reading purposes. Testers who freeze at JavaScript are usually freezing at syntax, not at meaning — and meaning is all you need. Read the verbs, skip the grammar." },
    { x: 20, y: 40, label: "Indentation shows nesting", desc: "Deeper means 'inside'. Follow the indentation to see what runs when. This alone lets you read a stack trace later and say which block was executing when everything caught fire." },
  ]}
/>
