      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            The Request/Response model of a single Selenium command.
          </p>
        </div>
        <button
          onClick={sendCommand}
    ...
    <div className="flex flex-col items-center gap-3">
      <motion.div
        ...
        className={`flex h-16 w-16 items-center justify-center rounded-2xl border-2 bg-background text-2xl shadow-xl transition-colors duration-300 ${
          isActive
            ? color + " shadow-" + color.split("-")[1] + "-500/20"
...
function getStepDescription(step: number) {
  switch (step) {
    ...
    case 1:
      return "Test code sends an HTTP request to the Driver.";
    case 2:
      return "Driver translates request into browser-specific protocol.";
    case 3:
      return "Browser executes the action on the DOM.";
    case 4:
      return "Browser sends success response back to the Driver.";
    case 5:
      return "Driver translates response back to the Test code.";
    default:
      return "";
  }
}
