...
<Term term="toil">
A term from Google's Site Reliability Engineering practice: work that is manual, repetitive,
automatable, tactical, devoid of enduring value, and that scales linearly with the size of
the thing you're supporting. The last property is the dangerous one — toil doesn't stay
constant, it grows with the system, so a team that doesn't attack it eventually spends all of
its time on it and none on the work it was hired for. Toil is not the same as "hard work" or
"boring work": the test is whether a machine could do it, and whether doing it teaches you
anything. If both answers are no, it's toil, and it will grow.
</Term>
