...
related:
  ...
  - "digital-literacy-and-safety/keyboard-and-typing/keyboard-layout"
...
<Term define={`When two handlers respond to the same key combination, and the outcome depends on which one sees the event first. The browser and operating system claim many combinations before your page ever sees them (\`Ctrl+W\`, \`Ctrl+T\`, \`Cmd+Q\`), and a web app cannot reliably override them. Collisions also happen *within* a page: a global single-key shortcut that doesn't check whether focus is inside a text field will fire while the user is typing. The defensive check is to ignore the event when \`document.activeElement\` is an \`input\`, \`textarea\`, or anything with \`contenteditable\` — and its absence is one of the most reliably reproducible bugs in any app that has shortcuts at all.`}>keyboard shortcut collision</Term>
