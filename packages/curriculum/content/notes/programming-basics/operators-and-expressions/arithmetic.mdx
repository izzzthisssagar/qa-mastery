...
<Term term="floating point">
The way computers store numbers with a fractional part, defined by the IEEE 754 standard.
A fixed number of bits holds a sign, an exponent and a fraction — so the value is stored as
a *sum of powers of two*, scaled. Any number that isn't such a sum (`0.1`, `0.2`, `0.7`)
can only be approximated, exactly as `1/3` can only be approximated in decimal. A 64-bit
`double` carries roughly 15–17 significant decimal digits. The consequences are permanent
and language-independent: never compare floats with `==` (compare the difference against a
small tolerance), never store money as a float (store integer pennies or use a decimal
type), and never assume a sum of many small floats is exact.
</Term>
