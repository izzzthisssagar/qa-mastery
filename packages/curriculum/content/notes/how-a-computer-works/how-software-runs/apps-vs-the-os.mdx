...
dissolve: <Term define="OS-enforced keys an app needs before touching protected hardware or data — camera, mic, location, files.">permissions</Term>, updates, "this app has been closed to protect your system" —
...
<FlowAnimation
  title="A permission request's journey — press Play"
  intervalMs={1500}
  nodes={[
    { label: "📱 App asks", desc: "'I'd like the camera, please.' The tenant files a request — it CANNOT just take the key. The architecture forbids it." },
    { label: "🎩 OS checks the cabinet", desc: "Has this app been granted the camera key before? Granted → proceed. Denied → refuse silently (the 'works everywhere but here' mystery). Never asked → escalate to the VIP." },
    { label: "🙋 You decide", desc: "The permission popup — the only moment the whole hierarchy pauses and asks the top floor. Your tap becomes policy." },
    { label: "🔒 OS enforces forever", desc: "The answer is stored and ENFORCED on every future access. Apps can re-ask; they can't override. Testing that they truly can't = real security QA." },
  ]}
/>

<CodePlayground
  language="python"
  title="Try it — the key cabinet as code"
  code={`# The OS's permission check, miniaturized. Edit the cabinet and re-run.
...
<WorkedExample title="the banking app that died after an OS update">
