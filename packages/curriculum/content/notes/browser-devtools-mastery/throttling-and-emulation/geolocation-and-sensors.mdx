...
<HotspotImage
  src="/notes/browser-devtools-mastery/throttling-and-emulation/PLACEHOLDER-sensors-panel.jpg"
  alt="Chrome DevTools Sensors panel open, showing a geolocation override with latitude and longitude fields and a preset city dropdown, plus orientation sliders below it"
  credit="Photo: PLACEHOLDER - replace with a screenshot of the DevTools Sensors panel showing geolocation override fields and orientation sliders together"
  pins={[
    { x: 50, y: 50, label: "Geolocation preset dropdown", desc: "A shortlist of world cities for a quick location swap. Fast for a first pass; doesn't test the exact address-level precision a delivery-radius or store-finder feature actually depends on." },
    { x: 50, y: 50, label: "Custom latitude, longitude, accuracy", desc: "Type an exact coordinate for a precise test - the corner of a specific delivery zone, or a point just outside a geo-restricted boundary, where the interesting bugs actually cluster." },
    { x: 50, y: 50, label: "Location unavailable option", desc: "Simulates the Geolocation API returning a permission-denied or hardware-failure error. The only realistic way to test your app's fallback UI without a real user's real, real-life permission denial." },
    { x: 50, y: 50, label: "Orientation sliders (alpha, beta, gamma)", desc: "Fake a compass heading and a physical tilt. Convincing for logic that reacts to orientation events; not a simulation of real gyroscope noise or drift on actual hardware." },
    { x: 50, y: 50, label: "What this panel does NOT touch", desc: "Your browser's real IP address, and therefore any server-side IP-based geolocation, is completely unaffected by anything in this panel. That gap is the single most common reason a geolocation override 'doesn't work' - it was never supposed to work on that layer." },
  ]}
/>
...
<Resources
  links={[
    ...
    { href: "PLACEHOLDER", title: "PLACEHOLDER video - catching a geo-restriction that checked the wrong mechanism", kind: "video" },
  ]}
/>

<Video url="PLACEHOLDER" title="Catching a geo-restriction that checked the wrong mechanism" minutes={5} />
