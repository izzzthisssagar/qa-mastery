...
<Term term="data-testid">
A `data-*` attribute conventionally used as a stable hook for automated tests. The browser
attaches no meaning to it, which is precisely why it's reliable: no styling change,
class rename, or CSS-in-JS hash regeneration can disturb it. It is a deliberate contract
between application code and test code. Its cost is that it appears in the shipped markup
and communicates nothing to users or assistive technology — so it complements role-based
locators rather than replacing them.
</Term>
