...
{/* IMAGE: a clothing store fitting room with a mirror, a hook of garments being tried on, price tags still attached */}
<HotspotImage
  src="/notes/browser-devtools-mastery/elements-and-styles/PLACEHOLDER-live-edit.jpg"
  alt="A fitting room mirror with several garments on hooks, price tags still attached"
  credit="Photo: PLACEHOLDER - replace with a fitting room interior, garments with tags on hooks and a large mirror"
  pins={[
    { x: 50, y: 50, label: "The tags stay on = nothing is purchased", desc: "Every DevTools edit keeps its price tag: you are trying, not buying. The server's HTML and CSS files are untouched no matter what you do in the panel. This is why live-editing is completely safe on production sites - you can only ever change YOUR browser's copy of the page." },
    { x: 50, y: 50, label: "The mirror = instant re-render", desc: "No waiting: change the text, nudge the padding, untick the declaration, and the page redraws immediately. The feedback loop for a layout hypothesis drops from 'file a ticket and wait' to about four seconds. Fast loops are what make real experimentation happen." },
    { x: 50, y: 50, label: "Trying combinations = hypothesis testing", desc: "Jacket with these trousers, then those: toggle one declaration at a time and watch what changes. If switching off ONE margin fixes the misalignment, you have isolated the offending rule - which is a dramatically better bug report than 'looks off'." },
    { x: 50, y: 50, label: "Leaving the cubicle = reload wipes everything", desc: "Walk out and the room resets for the next customer. Reload the page and every edit is gone - your rewritten copy, your toggled styles, your deleted banner. Ephemerality is the safety net AND the trap: screenshot your evidence BEFORE touching F5." },
    { x: 50, y: 50, label: "The house rules on the wall = the user agent stylesheet", desc: "Even an empty fitting room has rules you did not write. Every page inherits browser defaults - the 'user agent stylesheet' in the Styles pane - which is why headings are bold and paragraphs have margins nobody typed. When styling 'comes from nowhere', it comes from here." },
  ]}
/>
...
<Resources
  links={[
    ...
    { href: "PLACEHOLDER", title: "PLACEHOLDER - video: live-editing HTML and CSS as a testing technique", kind: "video" },
  ]}
/>

<Video href="PLACEHOLDER" title="PLACEHOLDER - fitting-room testing: live DOM and CSS edits for manual testers" minutes={9} />
