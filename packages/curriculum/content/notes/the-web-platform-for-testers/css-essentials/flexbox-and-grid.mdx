...
<Term define={`The flex property that decides how much an item gives up when the container is too small. It defaults to **1**, meaning every flex item is willing to shrink — which is why adding one card silently narrows all of them rather than overflowing. \`flex-shrink: 0\` refuses to shrink (the item keeps its width and something else overflows instead). Its partner \`flex-grow\` (default **0**) decides who absorbs *leftover* space. The shorthand \`flex: 1\` means \`flex-grow: 1; flex-shrink: 1; flex-basis: 0%\` — grow and shrink freely, ignoring content width — which is not at all the same as \`flex: auto\`, and the difference is responsible for a great deal of confusion.`}>flex-shrink</Term>
