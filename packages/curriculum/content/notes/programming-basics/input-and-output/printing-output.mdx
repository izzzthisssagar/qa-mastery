...
summary: "`print()` is the first line of code almost everyone writes, and it stays the single most-used debugging tool for the rest of your career. But printing hides two traps a tester must know: the difference between the value in memory and the text on screen, and the difference between stdout and stderr — the two separate channels every program writes to."
