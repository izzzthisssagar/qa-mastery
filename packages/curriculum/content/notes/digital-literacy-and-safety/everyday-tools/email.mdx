...
<Term term="SPF, DKIM and DMARC">
The three records a domain publishes in DNS to make its mail verifiable. **SPF** lists which
IP addresses may send mail for the domain. **DKIM** attaches a cryptographic signature that
the receiver verifies against a public key in DNS, proving the message wasn't altered.
**DMARC** ties them together: it tells receivers what to do when SPF or DKIM fail
(`p=none` — do nothing; `p=quarantine` — spam folder; `p=reject` — refuse it) and where to
send reports. The catch is that all three are *advisory*: a receiving server chooses whether
to honour DMARC, and a domain publishing `p=none` — which is the overwhelming majority — has
effectively published "please deliver forgeries of me anyway." Checking a domain's policy is
one command: `dig TXT _dmarc.example.com`.
</Term>
