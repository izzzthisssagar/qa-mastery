...
<Term term="type coercion">
Automatically converting a value from one type to another without being asked. Spreadsheets
coerce aggressively on input — text that resembles a number becomes a number, text that
resembles a date becomes a date serial — because it is right most of the time and being right
most of the time is a very good way to hide the times it isn't. The same phenomenon is why
`"1" + 1` is `"11"` in JavaScript and why an API that accepts `"0123"` may store `123`. The
defence is identical everywhere: declare the type explicitly instead of letting software
infer it, and never let inference happen at the boundary where data enters your system.
</Term>
