...
<Callout type="tip">
When a bug has sat untouched for days, the state tells you *what kind* of stuck it is — and the two
kinds need opposite responses. "In Progress" or "Retest" are **active** states: a person is (or should
be) working it, so a stall there is a workload or blocker question — ask the owner. "Pending Retest"
and "Assigned" are **queue** states: nobody is acting yet, the bug is just waiting its turn, so a stall
there just needs a nudge into the next step. Read the state before you chase the person.
</Callout>
