---
title: "Session & cache"
summary: "What a session actually is in storage terms — a cookie plus server state. Then the cache: the browser's photocopy pile of assets kept so pages load fast, until a deploy changes a file and the stale copy keeps being served. Why 'clear your cache' is a real mechanism, and how stale-asset bugs happen."
...
<HotspotImage
  src="/notes/browser-devtools-mastery/application-and-storage/PLACEHOLDER-cache-storage.jpg"
  alt="DevTools showing the Network tab Size column reading disk cache for reused assets, next to the Application panel Cache Storage list"
  credit="Photo: PLACEHOLDER - replace with a screenshot showing the Network tab Size column reading 'disk cache' on cached requests, plus the Application panel Cache Storage section with named caches listed"
  pins={[
    { x: 50, y: 50, label: "Size column says 'disk cache' = no network trip", desc: "In the Network tab, a request served from the cache shows the Size column reading (disk cache) or (memory cache) instead of a byte count, and it returns almost instantly. This is the photocopy being grabbed off the pile — the server was never contacted. When you're debugging a stale-asset bug, this column tells you at a glance which files came fresh and which came from last week's copy." },
    { x: 50, y: 50, label: "Cache-Control: max-age = the reuse window", desc: "Click a request and read its Response Headers for Cache-Control. max-age=3600 means the browser may reuse the copy for an hour without asking. A long max-age on a file whose URL never changes is the classic deploy trap: users keep the old copy until the window closes. Reading this header tells you HOW LONG a stale copy can haunt you." },
    { x: 50, y: 50, label: "Cache Storage = the service worker's pile", desc: "Application -> Storage -> Cache Storage lists NAMED caches filled by a service worker via the Cache API — a separate, programmable pile from the HTTP cache. Progressive web apps live here. If a site keeps serving old content even after an HTTP-cache clear, this pile (and the service worker, next note) is very often the reason." },
    { x: 50, y: 50, label: "Disable cache checkbox = test like a first visit", desc: "The Network tab's 'Disable cache' checkbox forces every request to the network for as long as DevTools is open — the browser ignores its pile entirely. This is how a tester reproduces a genuine first-visit or post-deploy load without nuking their whole cache. Tick it whenever you suspect a stale copy is hiding a bug or, worse, hiding a fix." },
    { x: 50, y: 50, label: "Hard reload = refetch this page's assets", desc: "With DevTools open, right-click the reload button for Empty Cache and Hard Reload. It throws out this page's cached assets and refetches them, which is the surgical version of 'clear your cache' for one page. It's the fastest way to answer 'is this bug real, or am I just looking at a stale copy?' — the first question to settle before you write anything up." },
  ]}
/>
...
<Resources
  links={[
    ...
    { href: "PLACEHOLDER", title: "PLACEHOLDER video — stale-asset bugs and clearing the cache in DevTools (replace with a real screencast URL)", kind: "watch" },
  ]}
/>

<Video href="PLACEHOLDER" title="Stale assets, Cache-Control, and clearing the cache" minutes={7} />
