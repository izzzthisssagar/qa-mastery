...
export const lessonFrontmatterSchema = z.object({
  ...
  lab_type: z.enum(["none", "test_design", "bug_report", "code_run", "artifact"]).default("none"),
  ...
  lab_language: z.enum(["java", "python", "javascript", "typescript", "csharp"]).optional(),
  flashcards: z.array(z.object({ front: z.string().min(1), back: z.string().min(1) })).default([]),
});
