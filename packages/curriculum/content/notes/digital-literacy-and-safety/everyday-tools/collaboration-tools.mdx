...
<Term term="asynchronous communication">
Communication that does not require both people to be present. Its defining property is that
**every unanswered question costs a full round-trip** — which, across timezones, is a day. A
message that omits the error text turns a five-minute fix into a two-day exchange, not
because anyone is slow, but because the reader must stop, ask, and wait. Writing
asynchronously well means front-loading every fact the reader could need, including the ones
you think are obvious, and accepting that six minutes of your writing is cheaper than one
round-trip of someone else's waiting. It is the core skill of distributed work, and it is
almost never taught.
</Term>
