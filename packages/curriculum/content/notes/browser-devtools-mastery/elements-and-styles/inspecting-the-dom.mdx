...
{/* IMAGE: a watchmaker or jeweler bent over a workbench, examining the open mechanism of a watch through a loupe, tools laid out */}
<HotspotImage
  src="/notes/browser-devtools-mastery/elements-and-styles/PLACEHOLDER-inspect-dom.jpg"
  alt="A watchmaker examining the exposed mechanism of a watch through a loupe at a workbench"
  credit="Photo: PLACEHOLDER - replace with a close-up of a watchmaker inspecting an open watch mechanism through a loupe"
  pins={[
    { x: 50, y: 50, label: "The loupe = the inspect cursor", desc: "The magnifier that turns 'the watch face' into 'this one gear, this one screw'. Cmd+Shift+C (or the box-with-arrow icon) is your loupe: hover anything on the page and the browser names the exact element, its tag, and its pixel size. You stop reporting on pages and start reporting on nodes." },
    { x: 50, y: 50, label: "The open case back = the live DOM", desc: "The printed spec sheet said what parts the watch SHOULD contain; opening the case shows what is actually in there now, including the replacement parts fitted since. Elements is the open case: the DOM after JavaScript added, removed, and rewrote nodes. Bugs are found in the mechanism, not the brochure." },
    { x: 50, y: 50, label: "The spec sheet on the bench = view-source", desc: "Still useful — it tells you what the server originally shipped, which matters for SEO checks, initial-load bugs, and 'was this ever in the HTML at all?' questions. But it never updates. If the page changed after load, the spec sheet is already out of date. Modern apps ship a nearly blank sheet and build everything on the wrist." },
    { x: 50, y: 50, label: "Touching one gear moves the hands = two-way highlight", desc: "Hover a node in the panel and the element glows on the page, wrapped in the blue-green-orange box-model overlay. Hover the page with the inspect cursor and the panel follows. This bidirectional link is how you match 'the thing that looks wrong' to 'the node that is wrong' in seconds." },
    { x: 50, y: 50, label: "The workbench light = per-tab scope", desc: "The loupe examines the watch on THIS bench only. DevTools inspects THIS tab: its DOM, its styles, its storage. The same site open in another tab is a different mechanism, possibly in a different state. When a bug reproduces in one tab and not another, the DOMs genuinely differ - inspect both." },
  ]}
/>
...
<Resources
  links={[
    ...
    { href: "PLACEHOLDER", title: "PLACEHOLDER - video walkthrough of inspecting the DOM as a tester", kind: "video" },
  ]}
/>

<Video href="PLACEHOLDER" title="PLACEHOLDER - Elements panel and the live DOM for manual testers" minutes={8} />
