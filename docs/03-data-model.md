...
score depends on (`quiz_attempts`, `xp_events`) has _read-own RLS and no write
policy_, so even with table-level `INSERT` granted, a learner's JWT cannot write
...
stayed hidden until the first `public` _write_ — `curriculum sync --apply` —
was run.
