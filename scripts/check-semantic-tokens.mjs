...
// purpose. See docs/superpowers/plans/2026-07-26-release-repository-governance.md
// Task 5.
...
const PASTEL_TEXT_RE = new RegExp(
  `\\btext-(?:${PASTEL_COLORS})-(?:50|100|150|200|300|400)\\b`,
  "g",
);

// Pre-existing findings from when this gate was added (Task 5, 2026-07) —
...
// Follow-up (tracked outside this task): swap these three to
...
function main() {
  const files = findSourceFiles(SRC_DIR).map((file) => ({
    file,
  ...
  if (stale.length > 0) {
    console.error(
      "These LEGACY_TOKEN_VIOLATIONS entries no longer exist — remove them from scripts/check-semantic-tokens.mjs:",
    );
