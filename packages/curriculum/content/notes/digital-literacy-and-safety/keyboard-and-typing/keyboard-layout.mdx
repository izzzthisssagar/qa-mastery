...
updated: "2026-07-10"
related:
  - "digital-literacy-and-safety/keyboard-and-typing/touch-typing"
  - "digital-literacy-and-safety/keyboard-and-typing/shortcuts"
  - "digital-literacy-and-safety/keyboard-and-typing/efficiency-tips"
---
...
<Term define={`The state of being the element that will receive keyboard input. Exactly one element per document has focus at a time, and the browser draws a **focus ring** around it. Native interactive elements (\`button\`, \`a href\`, \`input\`, \`select\`) are focusable automatically and sit in the **tab order**; \`<div>\` and \`<span>\` are not. \`tabindex="0"\` adds an element to the natural tab order, \`tabindex="-1"\` makes it focusable by script but not by Tab, and any positive \`tabindex\` overrides document order and is almost always a mistake. Removing the focus ring with \`outline: none\` — done constantly, for aesthetics — makes a page unusable for anyone navigating by keyboard, because they can no longer see where they are.`}>focus</Term>
