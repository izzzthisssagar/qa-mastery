...
<HotspotImage
  src="/notes/browser-devtools-mastery/audits-and-performance/PLACEHOLDER-lighthouse-report.jpg"
  alt="A laptop screen showing a Lighthouse report with five colored circular score gauges for Performance, Accessibility, Best Practices, SEO, and PWA above a scrollable list of individual named audits"
  credit="Photo: PLACEHOLDER - replace with a desk-level shot of a laptop screen showing a full Lighthouse report with five score gauges and an expanded audit list"
  pins={[
    { x: 50, y: 50, label: "The five gauges = five DIFFERENT rubrics, not one grade", desc: "Performance, Accessibility, Best Practices, SEO, and PWA each score against their own fixed checklist -- a 95 in SEO and a 40 in Performance are not contradictory, they are two unrelated report cards for the same page. Never average them in your head into one 'health number'; read each on its own terms." },
    { x: 50, y: 50, label: "The circle color = a threshold, not a curve", desc: "Green is 90-100, orange is 50-89, red is 0-49 -- fixed cutoffs applied to every page on earth, not a ranking against competitor sites. A page scoring orange isn't 'below average'; it's below a specific bar Google set for that category. Treat the color as a triage signal: red first, orange next, green rarely worth chasing further." },
    { x: 50, y: 50, label: "The audit list below the gauges = where the real information lives", desc: "Each row names a specific, fixable thing: an oversized image, a render-blocking script, a missing alt attribute. This list is what you paste into a bug report -- never the top-line score alone, which tells a developer nothing about WHERE to look or HOW MUCH it's costing." },
    { x: 50, y: 50, label: "Opportunities vs Diagnostics = estimated savings vs raw facts", desc: "Lighthouse splits Performance audits into 'Opportunities' (estimated time you'd save fixing this) and 'Diagnostics' (facts with no time estimate, like total byte weight). The estimated savings are ESTIMATES from a simulated throttled run -- useful for prioritizing, not a contract for how many milliseconds a fix will actually buy in production." },
    { x: 50, y: 50, label: "The device toggle above the gauges = mobile vs desktop, a different page entirely", desc: "Lighthouse simulates a mid-tier mobile CPU and a slow 4G-ish connection by default -- deliberately pessimistic, because that's closer to a real user's phone than your fiber-connected laptop. A page scoring 90 on desktop and 45 on mobile isn't lying twice; it's telling you the truth about two very different real audiences." },
  ]}
/>
...
<Video href="PLACEHOLDER" title="PLACEHOLDER -- Reading a Lighthouse report like a tester, not a scoreboard" minutes={8} />
