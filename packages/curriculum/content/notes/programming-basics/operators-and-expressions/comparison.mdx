...
<Term define={`A **value** is the data itself — the number \`42\`, the letters \`hello\`. A **reference** is the *address* where data lives. Java's primitives (\`int\`, \`double\`, \`char\`, \`boolean\`) hold values directly; everything else is a reference. So \`==\` compares whatever the variable actually holds: for a primitive that's the value (correct), and for an object that's the address (almost never what you want). Python is uniform — every variable is a reference — but its \`==\` operator is defined to compare values via \`__eq__\`, while \`is\` compares addresses. The lesson is identical in both: **decide whether you are asking "same thing" or "same contents", then use the operator that asks it.**`}>reference vs value</Term>
