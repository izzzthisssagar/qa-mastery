...
<Term term="specificity">
The tie-breaking rule the browser uses when two CSS rules both match one element and set
the same property. Scored as three numbers — **id count, class/attribute/pseudo-class
count, tag count** — compared left to right. `#nav .link` (1,1,0) beats `nav a.link.active`
(0,2,1) no matter which came later in the file, because the id column outranks everything
to its right. When a style "isn't applying," specificity is the usual culprit, and the
Elements → Styles panel shows the losing rule struck through.
</Term>
