    ...
    >
      <header className="relative flex items-center justify-between border-b border-border px-4 py-3">
        ...
        <button
          type="button"
          onClick={handleClose}
          className="rounded-lg px-2 py-1 text-muted-foreground transition-colors hover:bg-surface-raised hover:text-foreground"
        ...
        {messages.map((m, i) => (
          <div
            ...
            className={`max-w-[90%] rounded-2xl border px-3 py-2 text-sm leading-relaxed ${
              m.role === "user"
                ? "ml-auto border-accent/25 bg-accent/15 text-foreground"
                : "border-border/60 bg-surface-raised/80 text-foreground"
            }`}
          >
            {m.content}
          </div>
      ...
      >
        ...
        <button
          ...
          className="rounded-lg bg-accent px-3 py-2 text-sm font-medium text-accent-foreground transition-opacity disabled:opacity-40"
        >
          Send
        </button>
      </form>
