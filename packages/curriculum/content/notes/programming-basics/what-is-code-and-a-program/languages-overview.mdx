...
<FlowAnimation
  title="Two paths from code to running — interpreted vs compiled — press Play"
  intervalMs={1700}
  nodes={[
    { label: "⌨️ You write the same code", desc: "Either way, you start by typing instructions as plain text — the recipe. At this point Python and Java look equally inert: just words in a file. The difference is entirely in what happens NEXT to turn those words into actions." },
    { label: "🐍 Python: interpreter reads & runs it directly", desc: "The interpreted path (Python). A program called the interpreter reads your code and carries out each line right away, top to bottom. No separate build step — write a line, run, see the result. That immediacy is why Python feels fast to learn and experiment with." },
    { label: "☕ Java: compiler translates it ALL first", desc: "The compiled path (Java). A program called the compiler translates your entire file at once into a machine-ready form — and while doing so, catches some mistakes before anything runs. This is the extra up-front step: you compile, THEN you have something to run." },
    { label: "▶️ …then Java runs the translated version", desc: "Only after compiling does the program actually execute. So Java is a two-step trip (translate, then run) versus Python's one step (just run). More ceremony now, in exchange for speed and some errors caught early. Same destination, different route." },
    { label: "🎯 Both arrive at the same place: your instructions run", desc: "Interpreted or compiled, the end result is identical — the computer follows your instructions and produces output. The path differs; the nature of code does not. Knowing which path a language takes just tells you how it will FEEL to write and run." },
  ]}
/>
