...
<Term term="constraint validation">
The browser's built-in form checking, driven by attributes like `required`, `min`, `max`,
`pattern` and `type`. It runs before submit, blocks the request entirely when a rule
fails, and is queryable from the console via `form.checkValidity()` and the `:invalid`
CSS pseudo-class. It runs on the user's machine, which is why it is a usability feature
