...
<Term term="margin collapse">
The rule that adjoining vertical margins combine into a single margin equal to the
**largest** of them, rather than their sum. It applies between siblings, between a parent
and its first or last child, and to an empty element's own top and bottom margins. It never
applies horizontally, never to flex or grid items, and is blocked by any border, padding,
or overflow value other than `visible` sitting between the two margins. It is not a bug; it
is deliberate 1990s typography behaviour that makes consecutive paragraphs space evenly, and
it has confused every developer alive at least once.
</Term>
