...
-- ── capstone_submissions: already upserts on (user_id, note_slug) ───────────
-- Real gap: `created_at` is frozen at first submission and never updated by
-- the existing resubmission upsert (0033 / capstone-actions.ts), so there is
-- no signal for "last modified." `client_id` added for schema consistency
-- with the other notes tables and to support the same future offline-queue
-- correlation as note_progress; conflict resolution stays on
-- `unique(user_id, note_slug)` (0033), unchanged.
