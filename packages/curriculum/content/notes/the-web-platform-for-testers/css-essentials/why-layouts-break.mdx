...
<Term term="Cumulative Layout Shift (CLS)">
A Core Web Vital measuring how much visible content moves around *after* it first appears,
without the user causing it. Caused by images with no declared `width`/`height`, ads and
embeds injected into the flow, and web fonts swapping in with different metrics than the
fallback. It is felt, not seen: the user reaches for a button and it moves out from under
their finger as the page finishes loading. Google penalises it in ranking, which is the
only reason most teams ever fix it. Measure it in Lighthouse or the Performance panel;
reproduce it by throttling the network so the shifts happen slowly enough to watch.
</Term>
