...
export const NOTE_LABS: readonly NoteLab[] = [
  ...
  // ── API & Services ────────────────────────────────────────────────────────
  {
    chapterSlug: "api-testing-fundamentals/status-codes-and-rest",
    title: "Sort a request log by status family",
    brief:
      "Every status code belongs to a family — 2xx, 3xx, 4xx, 5xx. Count how many of each family showed up in this request log.",
    kind: "code_run",
    xp: 40,
    language: "python",
    starter: [
      ...
      "",
      "# Print one line per family, in this order, even if the count is 0:",
      "#   2xx: 3",
      "#   3xx: 1",
      "#   4xx: 2",
      "#   5xx: 2",
  ...
  {
    chapterSlug: "api-test-automation/contract-and-schema-testing",
    title: "Validate responses against the contract",
    brief:
      "The contract says every response needs id, name and email. Print the ids of the responses that break it.",
    kind: "code_run",
    xp: 45,
    language: "python",
    starter: [
      ...
      "]",
      "",
      "# Print the id of every response missing a required field, one per line:",
      "#   2",
      "#   4",
    ].join("\n"),
    checks: [
      { label: "Finds both broken responses", equals: "2\n4" },
      { label: "Checks against the full required field set", sourceContains: "issubset" },
      { label: "Validates every response", sourceContains: "for" },
    ],
  },
  {
    chapterSlug: "api-and-modern-security/jwt-and-token-attacks",
    title: "Hunt a token-handling bug on BuggyAPI",
    brief:
      "Probe BuggyAPI's auth for a real token issue — expiry not enforced, a scope that reaches further than it should, anything from this chapter. File one bug on it.",
    kind: "bug_report",
    xp: 65,
    target: "buggyapi",
    minValidBugs: 1,
  ...
  {
    chapterSlug: "docker-and-containers-for-testers/dockerfiles-and-compose",
    title: "Trace a Dockerfile's layer cache",
    brief:
      "Docker caches every layer up to the first one that changed, then rebuilds everything after it — even if the later lines didn't change. Print the layers that rebuild.",
    kind: "code_run",
    xp: 45,
    language: "python",
    starter: [
      ...
      '    \'CMD ["python", "app.py"]\',',
      "]",
      "changed_index = 3  # COPY . . changed",
      "",
      "# Print every line from changed_index onward — that's the cache",
      "# invalidation, layers before it stay cached:",
      "#   COPY . .",
      '#   CMD ["python", "app.py"]',
    ].join("\n"),
    checks: [
  ...
  {
    chapterSlug: "kubernetes-and-test-infrastructure/test-workloads-on-k8s",
    title: "Simulate a Job's backoffLimit",
    brief:
      "A Kubernetes Job gives up after backoffLimit retries, even if a later attempt would have passed. Simulate this run and print the outcome.",
    kind: "code_run",
    xp: 50,
    language: "python",
    starter: [
      ...
      "backoff_limit = 1",
      "",
      "# The Job gets backoff_limit + 1 total attempts before giving up.",
      "# Only look at that many attempts from the front of the list. Print:",
      '#   "Job SUCCEEDED after N attempts" if a pass shows up within the cap',
      '#   "Job FAILED after N attempts" otherwise (N = the attempt cap)',
      "# For this data: Job FAILED after 2 attempts",
    ].join("\n"),
    checks: [
      { label: "Reaches the correct verdict", equals: "Job FAILED after 2 attempts" },
      { label: "Caps the window at backoff_limit + 1", sourceContains: "backoff_limit" },
      { label: "Doesn't just scan the whole attempts list unbounded", sourceContains: "[:" },
    ],
  },
  {
    chapterSlug: "system-design-for-testers/scaling-building-blocks",
    title: "Trace a round-robin load balancer",
    brief:
      "A round-robin balancer sends each request to the next server in rotation, wrapping back to the first. Trace where 7 requests land across 3 servers.",
    kind: "code_run",
    xp: 45,
    language: "python",
    starter: [
      ...
      "num_requests = 7",
      "",
      "# Print one line per request, 1-indexed, in rotation order:",
      "#   request 1 -> web-1",
      "#   request 2 -> web-2",
      "#   ...",
      "#   request 7 -> web-1",
    ...
    checks: [
      {
        label: "Traces all 7 requests correctly",
        equals:
          "request 1 -> web-1\nrequest 2 -> web-2\nrequest 3 -> web-3\nrequest 4 -> web-1\nrequest 5 -> web-2\nrequest 6 -> web-3\nrequest 7 -> web-1",
      },
      { label: "Wraps around with modulo instead of a fixed cycle count", sourceContains: "%" },
    ],
  },
  {
    chapterSlug: "agile-and-devops-for-testers/tester-in-a-sprint",
    title: "Gate a story on its Definition of Done",
    brief:
      "A story can't move to Done until every DoD criterion is met. Check this story against the team's DoD and print the verdict.",
    kind: "code_run",
    xp: 40,
    language: "python",
    starter: [
      ...
      "}",
      "",
      "# If every criterion is met, print: DONE",
      "# Otherwise print NOT DONE, then each unmet criterion on its own line:",
      "#   NOT DONE",
      "#   docs updated",
    ].join("\n"),
    checks: [
      { label: "Reaches the correct verdict and lists the gap", equals: "NOT DONE\ndocs updated" },
      { label: "Checks every criterion against the DoD list", sourceContains: "for" },
    ],
  },

  // ── Specialized Testing ───────────────────────────────────────────────────
  {
    chapterSlug: "non-functional-testing-intro/compatibility",
    title: "Find the untested compatibility combos",
    brief:
      "The compatibility matrix says these browser/OS pairs must be covered. Print the ones nobody has tested yet.",
    kind: "code_run",
    xp: 40,
    language: "python",
    starter: [
      ...
      "",
      "# Print the untested combos, in required_matrix order, as browser/os:",
      "#   chrome/mac",
      "#   firefox/linux",
    ].join("\n"),
    checks: [
      { label: "Finds both untested combos", equals: "chrome/mac\nfirefox/linux" },
      { label: "Checks membership in the tested set", sourceContains: "tested_matrix" },
      { label: "Walks the required matrix in order", sourceContains: "for" },
    ],
  },
  {
    chapterSlug: "performance-testing/metrics",
    title: "Compute a real p95",
    brief:
      "An average hides your worst users. Compute the 95th percentile response time with the nearest-rank method and see how little one outlier moves it.",
    kind: "code_run",
    xp: 55,
    language: "python",
    starter: [
      ...
      "",
      "# Sort the data, then use the nearest-rank method for p95:",
      "#   rank = ceil(0.95 * n), then take the value at that rank (1-indexed)",
      "# Print just the number: 310",
    ].join("\n"),
    checks: [
  ...
  {
    chapterSlug: "security-testing-web/injection-and-client-side",
    title: "Hunt an injection bug on BuggyShop",
    brief:
      "Find a real reflected/stored XSS or injection point on BuggyShop and file it — this chapter's techniques applied for real, not just read about.",
    kind: "bug_report",
    xp: 65,
    target: "buggyshop",
    minValidBugs: 1,
  ...
  {
    chapterSlug: "accessibility-testing/automated-a11y-audits",
    title: "Flag the images an audit would catch",
    brief:
      "An automated a11y audit flags images with missing alt text — and the sneakier case of alt text that's just the filename, which is functionally useless to a screen reader. Find both.",
    kind: "code_run",
    xp: 45,
    language: "python",
    starter: [
      ...
      "]",
      "",
      "# Print the src of every image with empty alt text OR alt text that's",
      "# identical to its own filename, sorted:",
      "#   banner.jpg",
      "#   icon1.png",
    ].join("\n"),
    checks: [
  ...
  {
    chapterSlug: "mobile-testing/device-and-os-matrix",
    title: "Bucket screen widths into breakpoints",
    brief:
      "A responsive layout switches at three breakpoints. Given a batch of real device widths from a test run, classify each one.",
    kind: "code_run",
    xp: 45,
    language: "python",
    starter: [
      ...
      "",
      "# For each width, find the breakpoint whose range it falls in, and print:",
      "#   375: mobile",
      "#   768: tablet",
      "#   1440: desktop",
      "#   600: tablet",
    ...
    checks: [
      {
        label: "Buckets all five widths correctly, including both boundaries",
        equals: "375: mobile\n768: tablet\n1440: desktop\n600: tablet\n1023: tablet",
      },
      { label: "Checks the width against each breakpoint's range", sourceContains: "breakpoints" },
    ],
  },

  // ── Reporting & AI ─────────────────────────────────────────────────────────
  {
    chapterSlug: "test-management-and-reporting/metrics-and-reporting",
    title: "Flag a pass-rate regression",
    brief:
      "Three runs, three pass rates. Flag any run whose pass rate dropped from the run before it — that's the signal a dashboard should surface, not just the raw numbers.",
    kind: "code_run",
    xp: 45,
    language: "python",
    starter: [
      ...
      "]",
      "",
      "# Compute each run's pass rate. Whenever a run's rate is lower than the",
      "# PREVIOUS run's rate, print:",
      "#   run 2: 80.0% -> 78.0% (regression)",
      "# Run 1 has no previous run to compare to. Run 3 improved, so it prints",
      "# nothing.",
    ].join("\n"),
    checks: [
      { label: "Flags exactly the one regression", equals: "run 2: 80.0% -> 78.0% (regression)" },
      { label: "Compares each run against the one before it", sourceContains: "prev" },
    ],
  },
  {
    chapterSlug: "ai-and-the-modern-tester/ai-powered-test-automation",
    title: "Review AI test suggestions critically",
    brief:
      "An AI test-generation tool just suggested four cases. Some duplicate tests you already have — differently capitalized, differently spaced, same test. Sort the redundant suggestions from the genuinely new one.",
    kind: "code_run",
    xp: 50,
    language: "python",
    starter: [
      ...
      "]",
      "",
      "# Normalize each suggestion (lowercase, spaces -> underscores) and check",
      "# it against existing_tests. Print, in order:",
      "#   every redundant suggestion, using its ORIGINAL text",
      "#   then one summary line: N new suggestion: name, name, ...",
      "# For this data:",
      "#   test_login_valid_password",
      "#   Test Login Valid Password",
      "#   test_checkout_flow",
      "#   1 new suggestion: test_search_results",
    ].join("\n"),
    checks: [
      {
        label: "Sorts all four suggestions correctly",
        equals:
          "test_login_valid_password\nTest Login Valid Password\ntest_checkout_flow\n1 new suggestion: test_search_results",
      },
      { label: "Normalizes case and spacing before comparing", sourceContains: ".lower()" },
      { label: "Checks against the real existing set", sourceContains: "existing_tests" },
    ],
  },

  // ── Career ─────────────────────────────────────────────────────────────────
  {
    chapterSlug: "interviews/technical-rounds",
    title: "Find the first duplicate in a run log",
    brief:
      "A classic technical-round question, QA-flavored: a CI run log has a test ID that got submitted twice. Find the first one that repeats, scanning left to right.",
    kind: "code_run",
    xp: 45,
    language: "python",
    starter: [
      ...
      "",
      "# Print the FIRST id that shows up a second time, scanning left to",
      "# right (not the most frequent one — the first repeat encountered):",
      "#   run-2",
    ].join("\n"),
    checks: [
      { label: "Finds the first duplicate, not just any duplicate", equals: "run-2" },
      { label: "Tracks what's been seen so far", sourceContains: "seen" },
    ],
  },
  {
    chapterSlug: "a-portfolio-that-gets-interviews/the-3-repo-portfolio",
    title: "Lint a portfolio README",
    brief:
      "A README that sells needs specific sections. Check a submitted README's headings against the checklist and print what's missing.",
    kind: "code_run",
    xp: 40,
    language: "python",
    starter: [
      ...
      "",
      "# Print the required sections that are missing from readme_headings,",
